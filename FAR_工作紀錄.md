...
#### 個人資料新增父母撫養欄位
- **新增欄位：** 「需要撫養父母」YES/NO 單選 + 「父母年齡（最長壽）」數字輸入
- **UI 行為：** 預設 否（No），選「是」後動態顯示年齡輸入框；選「否」時隱藏並清空年齡
- **三語 i18n：** `lParentSupport` / `lParentAge` / `optYes` / `optNo` — 中/英/BM 全支援
- **儲存/載入：** hidden input `id="parentSupport"` 存入 localStorage；loadRecord() 自動還原 radio 狀態及顯示邏輯
- **示範資料：** loadDemo() 預設 是（Yes），父母年齡 68 歲
- **清空：** clearAll() 重設 radio 回「否」並隱藏年齡欄位
